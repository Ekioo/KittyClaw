...
- `TicketWorktreeService` — follows the ticket parent chain, then creates `<repo>.worktrees/ticket-<root-id>` on branch `ticket/<root-id>` or reuses the local branch already registered at that canonical path. A non-canonical registered branch is accepted only while the worktree is clean, so the whole ticket family keeps the existing delivery branch without absorbing uncommitted changes. It returns both the canonical worktree and primary repository paths; missing or unregistered worktrees fail resolution before a provider starts.
- `AgentRunner` — keeps loading skills and `.agents/**` from the control workspace while launching the provider process in the resolved worktree. The resolved directory is retained across provider fallback and steering replay, persisted on the run, and exposed by the runs API as `workingDirectory`. Runs sharing a worktree are serialized; runs for different roots may execute concurrently. For a worktree-bound run, the runner fingerprints tracked changes and untracked file contents in the primary repository before and after execution (orchestrator-owned `.agents/channel/**` state and consolidated agent memory are excluded). Fingerprint drift alone never fails the run: the run fails as a boundary violation only when the agent's own tool stream declares a write into the primary checkout (a write-capable file tool targeting a path that actually changed, a mutating `git` invocation referencing the primary repository, or a shell command naming a changed path — `PrimaryCheckoutChangeAttribution`). Drift that overlaps a local-checkout synchronization window (`PrimaryCheckoutActivityRegistry`, fed by `WorktreeMergeQueueService`) is reported as a coordinated KittyClaw change; any other drift is reported as an external change of undetermined origin, with the changed paths, without failing the run and without imputing it to the agent.
- `WorktreeMergeQueueService` — persists integration requests per project, processes them in creation order, rebases against a durable per-target integration tip, validates the result in a dedicated detached-HEAD integration worktree, and atomically advances a ref that is never checked out. The primary checkout and its branch remain unchanged; a separate synchronization step catches them up. It automatically union-merges rebases whose unresolved paths are exclusively append-only processor memory indexes or lesson files, while preserving every mixed or non-memory conflict for manual review without deleting the ticket worktree. It supports explicit resume and recovers interrupted processing after restart.
