...
- `KittyClaw.Web/Components/ChatDrawer.razor` — new-instruction drawer that stores the selected model in browser `localStorage` under a project-specific key. On first render it restores the stored value only when that model is still available; otherwise it keeps the default model.
