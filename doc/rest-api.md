...
- `GET /api/projects/{slug}/chat/model?target={target}` — returns the model bound to an existing chat conversation, or `null` when the conversation has no model binding.
