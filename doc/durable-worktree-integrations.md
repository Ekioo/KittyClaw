...
The queue never modifies a target branch while its primary checkout has local changes. Isolated runs continue, while the integration becomes `BlockedByExternalChanges` and remains visible on the project tile. The background reconciler retries it after the external changes are resolved. It also discovers registered worktrees for terminal root tickets after startup and queues them without creating missing worktrees. When the next pending job belongs to a ticket family that is still busy (active agent runs or processor writes), the queue skips it and integrates the next independent job instead of stalling behind it; the skipped job stays `Pending` and is retried later.
