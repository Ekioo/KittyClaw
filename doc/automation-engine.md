...
- **First-match-wins per ticket per tick** (ticket #112): automations are evaluated in `automations.json` file order; within one tick, the first `ticketInColumn` automation that matches a ticket AND passes its conditions consumes it — later column-poll automations skip that ticket for the tick (logged). Order route-style automations (e.g. an assigner for unassigned tickets) BEFORE dispatch-style ones in the file. A ticket not matched (trigger filter or conditions) stays available to later automations in the same tick.
