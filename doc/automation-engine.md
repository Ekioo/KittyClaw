...
- A `runAgent` action fired without a ticket follows the same durable-write contract: it runs in the registered maintenance worktree, commits only its `versionedWritePaths` (default `.agents`, `tools`, `scripts`), and enters the normal merge queue. Failed or stopped agents keep their isolated output for review. Ticket-bound agents continue to use their ticket worktree.
